...
- For UI, design-system, typography, CSS architecture, Vue component, Histoire, and LiveVue runtime guidance, consult `DESIGN.md` alongside this file
- For signed-in browser UI, follow the LiveView-first LiveVue contract in `DESIGN.md`: LiveView owns route state, persisted state, validation, authorization-sensitive actions, mutations, and real-time updates; Vue owns presentation and local-only interaction state
