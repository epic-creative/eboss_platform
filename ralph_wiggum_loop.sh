...
usage() {
  cat <<'USAGE'
Usage: ./ralph_wiggum_loop.sh [options]
...
Options:
  ...
  --model MODEL           Primary model for codex exec (default: gpt-5.3-codex-spark)
...
Examples:
  ...
  ./ralph_wiggum_loop.sh --start-at ST-ONB-001 --max 3
  ./ralph_wiggum_loop.sh --only ST-ONB-001 --no-push
USAGE
}
...
build_codex_cmds() {
  local primary_effort_config
  local fallback_effort_config
  primary_effort_config="$(printf 'model_reasoning_effort="%s"' "$PRIMARY_REASONING_EFFORT")"
  fallback_effort_config="$(printf 'model_reasoning_effort="%s"' "$FALLBACK_REASONING_EFFORT")"

  CODEX_PRIMARY_CMD=(codex exec --cd "$REPO_ROOT" --full-auto --model "$CODEX_MODEL" -c "$primary_effort_config")
  CODEX_FALLBACK_CMD=(codex exec --cd "$REPO_ROOT" --full-auto --model "$FALLBACK_CODEX_MODEL" -c "$fallback_effort_config")

  if [ -n "$CODEX_PROFILE" ]; then
    CODEX_PRIMARY_CMD+=(--profile "$CODEX_PROFILE")
    CODEX_FALLBACK_CMD+=(--profile "$CODEX_PROFILE")
  fi

  if [ "${#CODEX_EXTRA_ARGS[@]}" -gt 0 ]; then
    CODEX_PRIMARY_CMD+=("${CODEX_EXTRA_ARGS[@]}")
    CODEX_FALLBACK_CMD+=("${CODEX_EXTRA_ARGS[@]}")
  fi
...
run_precommit_with_fix_loops() {
  ...
  local story_title="$2"
  local attempt=0
  local fallback_fix_attempted=0
  local precommit_log
  local fix_prompt
  ...
  while true; do
    ...
    if [ "$attempt" -ge "$MAX_FIX_ATTEMPTS" ]; then
      if [ "$fallback_fix_attempted" -eq 1 ]; then
        log "mix precommit still failing for ${story_id} after primary + fallback fix runs"
        rm -f "$precommit_log"
        return 1
      fi

      fallback_fix_attempted=1
      log "mix precommit still failing for ${story_id}; switching to fallback model ${FALLBACK_CODEX_MODEL} (${FALLBACK_REASONING_EFFORT})"
    else
      attempt=$((attempt + 1))
    ...
    fix_prompt="$(mktemp "${TMPDIR:-/tmp}/ralph-fix-prompt.XXXXXX")"
    {
      ...
      if [ "$fallback_fix_attempted" -eq 1 ]; then
        echo "- use deeper reasoning and root-cause debugging to break out of prior stuck attempts"
      fi
      echo
    ...
    if [ "$fallback_fix_attempted" -eq 1 ]; then
      if ! run_codex_prompt "$fix_prompt" "fallback"; then
        rm -f "$fix_prompt" "$precommit_log"
        return 1
      fi
    elif ! run_codex_prompt_with_fallback "$fix_prompt" "precommit fix loop for ${story_id}"; then
      rm -f "$fix_prompt" "$precommit_log"
      return 1
    fi
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --primary-effort)
      require_next_arg "$1" "$#"
      require_non_flag_value "$1" "$2"
      PRIMARY_REASONING_EFFORT="$2"
      shift 2
      ;;
    --fallback-model)
      require_next_arg "$1" "$#"
      require_non_flag_value "$1" "$2"
      FALLBACK_CODEX_MODEL="$2"
      shift 2
      ;;
    --fallback-effort)
      require_next_arg "$1" "$#"
...
while [ "${#pending_stories[@]}" -gt 0 ]; do
  ...
  for record in "${pending_stories[@]}"; do
    ...
    if ! run_codex_prompt_with_fallback "$prompt_file" "story implementation for ${story_id}"; then
      rm -f "$prompt_file"
      fail "codex exec failed for $story_id"
    fi

    if [ -z "$(git status --porcelain)" ] && [ "$LAST_CODEX_RUN_MODE" = "primary" ]; then
      log "No changes detected after primary run for ${story_id}; retrying with fallback model ${FALLBACK_CODEX_MODEL}"
      LAST_CODEX_RUN_MODE="fallback"
      if ! run_codex_prompt "$prompt_file" "fallback"; then
        rm -f "$prompt_file"
        fail "codex fallback exec failed for $story_id"
      fi
    fi
    ...
    if [ -z "$(git status --porcelain)" ]; then
      fail "No changes detected after codex primary/fallback loop for $story_id"
    fi
